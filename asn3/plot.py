...
# Plots a partitioned circuit given a name, nets, best cost, and partitioned circuit
def plot(name, nets, mincut, partitioned_circuit=None):
    # If partitioned_circuit input is None, read from saved partitioned circuit
    ...
    # First two is for determining odd total size partition circuits
    # Last one is for even partitions
    ...
    # Create colour map and number of individual colours for nets
    cmap = plt.get_cmap('nipy_spectral')
    colours = [cmap(i) for i in np.linspace(0, 1, len(nets))]

    # Draw all nets
    for i, net in enumerate(nets):
        # Same colour for each net
...
# Helper function to save best partitioned circuit
def save(input):
